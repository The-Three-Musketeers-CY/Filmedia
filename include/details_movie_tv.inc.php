<?php
    ...
    foreach ($media->genres as $genre) {
        if ($i != 0) $genres .= ", ";
        $genres .= htmlspecialchars($genre->name);
        $i++;
    }
        ...
        <section id="details">
            ...
            <div id="details-info">
                ...
                <div id="details-info-description">
                    <p><?php echo htmlspecialchars($media->overview); ?></p>
                </div>
<?php
    if (isset($media->{"watch/providers"}->results->FR)) {
        ...
        if (isset($providers->buy)) {
            ...
            foreach ($providers->buy as $provider) {
                echo "\t\t\t<img src=\"https://image.tmdb.org/t/p/w45" . $provider->logo_path . "\" alt=\"logo de " . htmlspecialchars($provider->provider_name) . "\"/>\n";
            }
            echo "\t\t</div>\n";
        }
    ...
    if (isset($media->credits->cast) && count($media->credits->cast) > 0) {
        ...
        while ($i < 4 && $i < count($casts)) {
            ...
            if (isset($cast->profile_path)) {
                echo "\t\t\t\t<img src=\"https://image.tmdb.org/t/p/w92" . $cast->profile_path . "\" alt=\"Photo de " . htmlspecialchars($cast->name) . "\"/>\n";
            } else {
                echo "\t\t\t\t<img src=\"./img/no-image.svg\" width=\"92\" alt=\"no image\"/>\n";
            }
            echo "\t\t\t\t<h4>" . htmlspecialchars($cast->name) . "</h4>\n";
            echo "\t\t\t\t<p>" . htmlspecialchars($cast->character) . "</p>\n";
            echo "\t\t\t</article>\n";
            echo "\t\t</a>\n";
            $i++;
        }
...
<?php
    ...
    while($i < count($media->videos->results) && $found == false) {
        ...
        if ($video->type == "Trailer") {
            ...
            echo "\t<iframe width=\"560\" height=\"315\" src=\"https://www.youtube.com/embed/" . $video->key . "\" title=\"Bande annonce de " . $title . "\" allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen=\"allowfullscreen\"></iframe>\n";
            echo "</article>\n";
            $found = true;
        }
        $i++;
...
<?php
    ...
    foreach ($similars as $similar) {
        if ($type == "movie") {
            $title = htmlspecialchars($similar->title);
            $date = $similar->release_date;
        } else {
            $title = htmlspecialchars($similar->name);
